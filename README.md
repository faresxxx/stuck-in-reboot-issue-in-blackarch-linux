...
this means that GRUB is not detecting an actual bootable operating system, and it is only adding an option to boot into the EFI firmware settings (BIOS/UEFI setup). This typically occurs if:
    ...
    The GRUB installation is incomplete or misconfigured.
    The root partition is not correctly detected by GRUB.
    GRUB is not finding the kernel and initramfs files (e.g., vmlinuz-linux and initramfs-linux.img).
