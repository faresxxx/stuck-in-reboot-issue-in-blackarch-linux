...
>## error found memtest86+ image /usr/bin/grub-probe warning uknown device type nvme0n1

## 1. Ensure NVMe Support in GRUB

**1. Rebuild the Initial Ramdisk Environment :**

    mkinitcpio -P
 
 **2. Ensure GRUB Has NVMe Support :**

     nano /etc/default/grub
In this file, find the line that begins with `GRUB_PRELOAD_MODULES`. If it doesn’t exist, add it at the bottom:

    GRUB_PRELOAD_MODULES="nvme"
  **3. Reinstall GRUB**

     grub-install --target=x86_64-efi --efi-directory=/efi --bootloader-id=GRUB

## 2. Check Partition Labels and UUIDs
Sometimes, the error is caused by an incorrect UUID or partition identifier in your GRUB or `/etc/fstab` configuration.
- Use `blkid` to list the PARTUUIDs for your partitions
- Ensure that the `PARTUUID` in your GRUB config (`/etc/default/grub`) and in `/etc/fstab` matches the actual root partition’s PARTUUID.

**1. Edit GRUB Config :**

    nano /etc/default/grub

Ensure that the `GRUB_CMDLINE_LINUX_DEFAULT` line has the correct `PARTUUID` for the root partition. It should look something like this:

    GRUB_CMDLINE_LINUX_DEFAULT="root=PARTUUID=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx rw"

Replace the `PARTUUID` with the actual PARTUUID of your root partition from the `blkid` output.
- After verifying the UUIDs and editing `/etc/default/grub`, regenerate the GRUB config

**2. Reboot and Test**

>## GRUB_CMDLINE_LINUX_DEFAULT="root=PARTUUID=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
>### rw"GRUB_CMDLINE_LINUX_DEFAULT="loglevel= quiet"

the `GRUB_CMDLINE_LINUX_DEFAULT` entry in your `/etc/default/grub` file currently only includes logging and quiet boot options. To resolve the issue related to your root partition and ensure that GRUB knows which partition to boot from, you'll need to include the correct `root=PARTUUID=` parameter in this line.
 **1. Find the Correct PARTUUID :** 
 First, you need to determine the correct `PARTUUID` for your root partition.   Run the following command:
 
    blkid

This will list all partitions with their corresponding `UUID` and `PARTUUID`. Find the `PARTUUID` for your root partition (this is the partition where your root filesystem `/` is located). It should look something like this:

    /dev/nvme0n1p3: UUID="xxxx-xxxx" TYPE="ext4" PARTUUID="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"

Take note of the `PARTUUID` value for your root partition.

**2. Modify `/etc/default/grub`**

    nano /etc/default/grub
Modify the line to look like this (replace `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` with the actual `PARTUUID` for your root partition):

    GRUB_CMDLINE_LINUX_DEFAULT="root=PARTUUID=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx rw loglevel=quiet"

**3. Regenerate GRUB Configuration and reboot the system**
