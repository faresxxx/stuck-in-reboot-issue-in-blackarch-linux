...
**6. Use `bootx64.efi` Instead of Kernel Directly :** In many cases, UEFI Shell will not allow you to run the kernel (`vmlinuz-linux`) directly because it’s not a standard EFI executable. Instead, you need to run a boot loader like GRUB or `systemd-boot`.
